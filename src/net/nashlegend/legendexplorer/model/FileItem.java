package net.nashlegend.legendexplorer.model;

import java.io.File;
import java.net.URI;

import net.nashlegend.legendexplorer.application.ExplorerApplication;

import net.nashlegend.legendexplorer.R;
import net.nashlegend.legendexplorer.R.drawable;
...
public class FileItem extends File {
	...
	private void setFileTypeBySuffix() {
		if (isDirectory()) {
			...
			// 根据后缀获取文件类型
			if (isArrayContains(
					getStringArraySourceByName(R.array.TypePackage), suffix)) {
				setFileType(FILE_TYPE_APK);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeImage), suffix)) {
				setFileType(FILE_TYPE_IMAGE);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeAudio), suffix)) {
				setFileType(FILE_TYPE_AUDIO);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeVideo), suffix)) {
				setFileType(FILE_TYPE_VIDEO);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeText), suffix)) {
				setFileType(FILE_TYPE_TXT);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeZip), suffix)) {
				setFileType(FILE_TYPE_ZIP);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeExcel), suffix)) {
				setFileType(FILE_TYPE_EXCEL);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeHTML), suffix)) {
				setFileType(FILE_TYPE_HTML);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypePDF), suffix)) {
				setFileType(FILE_TYPE_PDF);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypePPT), suffix)) {
				setFileType(FILE_TYPE_PPT);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeWord), suffix)) {
				setFileType(FILE_TYPE_WORD);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeChm), suffix)) {
				setFileType(FILE_TYPE_CHM);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeTorrent), suffix)) {
				setFileType(FILE_TYPE_TORRENT);
			} else if (isArrayContains(
					getStringArraySourceByName(R.array.TypeEBook), suffix)) {
	...
	private String[] getStringArraySourceByName(int id) {
		return ExplorerApplication.getGlobalContext().getResources()
				.getStringArray(id);
	}
